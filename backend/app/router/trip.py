...
async def run_orchestrator_background(trip_id: str, destination: str, trip_duration_days: int, selected_dates: str):
    """
    ...
    try:
        ...
        await broadcast_agent_status("Destination Research Agent", "running", "Researching destinations and attractions")
...
async def create_trip(body: CreateTripRequest):
    ...
    try:
        ...
        print(f"[create_trip] Created trip: {trip.trip_name} (code: {trip.trip_code}, id: {trip_id})")
...
async def join_trip(body: JoinTripRequest):
    ...
    try:
        ...
        if not trip_doc:
            raise HTTPException(status_code=404, detail=f"Trip with code {body.trip_code} not found")
        ...
        for user_id in trip_doc.get("members", []):
            ...
            if user:
                member_details.append({
                    ...
                    "has_submitted_preferences": user_id in trip_doc.get("members_with_preferences", [])
                })
...
async def trigger_all_in(trip_id: str):
    ...
    try:
        ...
        if len(members_with_prefs) < len(members):
            ...
            warnings.append(f"Not all members have submitted preferences. Missing: {len(missing)} member(s)")
...
async def delete_trip(trip_id: str, user_id: str = Query(..., description="User ID requesting deletion")):
    ...
    try:
        ...
        if user_id not in trip.get("members", []):
            return APIResponse(
                code=403,
                msg="You are not a member of this trip",
                data=None
            )
        ...
        # If user is a regular member, just remove them from the trip
            ...
            result = await trips_collection.update_one(
                {"_id": ObjectId(trip_id)},
                {
                    "$pull": {
                        "members": user_id,
                        "members_with_preferences": user_id
                    }
                }
            )
            ...
            if result.modified_count > 0:
                ...
                return APIResponse(
                    code=0,
                    msg="You have left the trip",
                    data={"trip_id": trip_id}
                )
            else:
                return APIResponse(
                    code=500,
                    msg="Failed to leave trip",
                    data=None
                )
