...
class Trip(BaseModel):
    ...
    trip_code: str = Field(
        default_factory=generate_trip_code,
        description="Unique 6-character trip code for joining"
    )
    ...
    members: list[str] = Field(
        ...
        description="List of user IDs who have joined the trip"
    )
    members_with_preferences: list[str] = Field(
        default_factory=list,
        description="List of user IDs who have submitted preferences"
    )
    ...
    trip_duration_days: int | None = Field(None, description="Most common duration from preferences")
